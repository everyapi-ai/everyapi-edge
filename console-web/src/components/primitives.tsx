...
export const Panel = ({
  ...
  <section
    className={`rounded-xl border border-line bg-surface-0 p-5 shadow-[0_16px_40px_-30px_rgba(0,0,0,0.9)] ${className}`}
  >
    {title ? (
      <h2 className='mb-4 text-base font-semibold tracking-[-0.01em] text-ink'>{title}</h2>
    ) : null}
    {children}
  </section>
)
...
export const StatCard = ({
  ...
  <article className='relative overflow-hidden rounded-xl border border-line bg-surface-0 p-4 shadow-[0_14px_32px_-28px_rgba(0,0,0,0.9)]'>
    ...
    <div className='mt-2.5 text-[28px] leading-none font-semibold tracking-[-0.03em] text-ink'>
      {value}
    </div>
