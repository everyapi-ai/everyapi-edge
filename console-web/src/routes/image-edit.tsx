...
const ImageEditPage = () => {
  ...
  const submit = async () => {
    ...
    try {
      const response = await fetch('/api/image/edit', { method: 'POST', body: form, signal: controller.signal })
      ...
      if (!controller.signal.aborted) setError(reason instanceof Error ? reason.message : t('imageEdit.error'))
    } finally {
  ...
  const runtimeError = runtime.data?.error === 'A CUDA-capable GPU is required for image editing.'
  ...
  return <div className='flex flex-col gap-5'>
    ...
    <Panel title={t('imageEdit.title')}>
      {!ready ? <p data-image-runtime-error className='mb-4 text-sm text-muted'>{runtimeError}</p> : null}
      <div className='grid gap-4 lg:grid-cols-2'>
        <div className='flex flex-col gap-3'>
          ...
          >
            ...
            <span className='min-w-0'>
              ...
              <span data-source-file-name className='mt-0.5 block truncate font-mono text-xs text-muted'>{image?.name || t('imageEdit.noSource')}</span>
            </span>
          </label>
          <input id='source-image' aria-label={t('imageEdit.source')} type='file' accept='image/*' onChange={(event) => selectSourceImage(event.target.files?.[0])} className='sr-only' />
          {sourcePreview && image ? (
            <figure className='rounded-md border border-line bg-surface-1 p-2.5'>
              <img data-image-source-preview src={sourcePreview} alt={image.name} className='max-h-56 w-full rounded-sm object-contain' />
              <figcaption className='mt-2 truncate font-mono text-xs text-muted'>{image.name}</figcaption>
            </figure>
          ) : null}
          <p className='text-xs text-muted'>{t('imageEdit.activeModel')}: <span data-image-editor-model className='font-mono text-ink'>{model}</span></p>
          ...
          {pending ? (
            <Button type='button' variant='ghost' data-stop-image-edit onClick={stop} className='gap-2'><ImageUp className='size-4' />{t('imageEdit.stop')}</Button>
          ) : (
            <Button type='button' disabled={!ready || !image || !prompt.trim()} onClick={() => void submit()} className='gap-2'><ImageUp className='size-4' />{t('imageEdit.submit')}</Button>
          )}
        ...
        <div className='rounded-md border border-dashed border-line p-3'>
          {result ? (
            <div>
              <img src={result} alt={t('imageEdit.result')} className='max-h-[520px] w-full object-contain' />
              <a href={result} download='everyapi-image-edit.png' className='mt-3 inline-flex text-sm font-medium text-accent hover:text-ink'>
