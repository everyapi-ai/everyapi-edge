...
const RuntimePage = () => {
  ...
  const releaseAll = () => {
    ...
    if (
      count &&
      window.confirm(t('runtime.unloadAllConfirm', { count: formatCount(count, locale) }))
    )
      unloadAll.mutate()
  }
  ...
  const loadedPercent = totalMemoryBytes
    ? Math.min(100, Math.round((loadedMemoryBytes / totalMemoryBytes) * 100))
    : 0
  const reservedPercent = totalMemoryBytes
    ? Math.min(100 - loadedPercent, Math.round((reservedMemoryBytes / totalMemoryBytes) * 100))
    : 0
            ...
            {overview.data?.vram_total_gb ? (
              <section data-runtime-memory-budget className='mt-5 border-t border-line pt-4'>
                ...
                >
                  ...
                  />
                  <span
                    className='bg-warn/80 transition-[width] duration-300'
                    ...
                    {runtime.data.models.map((model) => (
                      <tr key={model.name}>
                        ...
                        <td className='border-b border-line px-3 py-3 text-ink-2'>
                          {formatVRAMGigabytes(model.size_vram)}
                        </td>
                        <td className='border-b border-line px-3 py-3 text-ink-2'>
                          {model.context_length
                            ? formatCount(model.context_length, locale)
                            : t('common.unknown')}
                        </td>
                        <td className='border-b border-line px-3 py-3 text-ink-2'>
                          {formatTime(model.expires_at, locale)}
                        </td>
