...
export const ImageEditPlayground = ({
  capability,
  embedded = false,
}: {
  ...
  const runtime = useImageRuntime(!capability)
  ...
  const model = capability ? (capability.models[0] ?? '') : (runtime.data?.models[0] ?? '')
  ...
  const submit = async () => {
    ...
    try {
      const response = await apiFetch('/api/image/edit', {
        method: 'POST',
        body: form,
  ...
  const ready = capability ? capability.status === 'ready' : runtime.data?.status === 'ready'
  const rawRuntimeError = capability?.reason || runtime.data?.error
  const runtimeError =
    rawRuntimeError === 'A CUDA-capable GPU is required for image editing.'
      ? t('imageEdit.gpuRequired')
      : rawRuntimeError || t('imageEdit.unavailable')
  return (
