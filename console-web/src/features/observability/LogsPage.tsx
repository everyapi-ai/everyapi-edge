...
export const LogsPage = () => {
  ...
  const levels = useMemo(
    () =>
      [...new Set((logs.data ?? []).map((entry) => entry.level.toLowerCase()))].sort(
    ...
    <div className='flex flex-col gap-5'>
      ...
      <Panel title={t('logs.title')}>
        ...
        >
          <div
            data-log-filters
            className='mb-4 grid gap-3 sm:grid-cols-[minmax(0,0.55fr)_minmax(0,1.45fr)]'
          >
            <div>
              <label htmlFor='log-level' className='mb-1.5 block text-xs font-medium text-ink-2'>
                {t('logs.filterLevel')}
              </label>
              <select
                id='log-level'
            ...
            <div>
              <label htmlFor='log-search' className='mb-1.5 block text-xs font-medium text-ink-2'>
                {t('logs.search')}
              </label>
              <Input
                id='log-search'
          ...
          <p data-log-count className='mb-3 text-xs text-faint'>
            {t('logs.matching', { count: filteredLogs.length })}
          </p>
          <ol className='max-h-[60vh] overflow-auto rounded-lg border border-line bg-surface-1 px-4'>
            {filteredLogs.map((entry, index) => (
              ...
              >
                <time className='mr-2 font-mono text-xs text-faint'>
          ...
          {filteredLogs.length === 0 ? (
            <p data-log-empty className='py-5 text-sm text-muted'>
              {t('logs.noMatches')}
            </p>
          ) : null}
        </QueryState>
