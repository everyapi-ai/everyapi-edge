...
func LoadOrGenerate(path string) (Decoded, error) {
	...
	if err := assertSafeIdentityFile(expanded); err != nil {
		return Decoded{}, err
	}
...
func assertSafeIdentityFile(path string) error {
	...
	if err != nil {
		...
		return fmt.Errorf("stat identity %s: %w", path, err)
	}
	if info.Mode()&os.ModeSymlink != 0 {
		return fmt.Errorf(
			"identity %s is a symlink; refusing to follow it (resolve the link and place the real file at %s)",
			path, path,
		)
	}
	...
	if mode := info.Mode().Perm(); mode&0o077 != 0 {
		return fmt.Errorf(
			"identity %s has too-permissive mode %#o; expected 0600 (chmod 600 %s and retry)",
			path, mode, path,
		)
	}
	return nil
